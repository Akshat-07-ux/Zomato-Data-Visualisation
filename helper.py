...
def most_popular_cuisines(df3,c):
  fig = px.bar(df3, x=c.index, y=c.values, labels={'x': 'cuisines', 'y': 'count'})
  return fig
...
def state_famous_cuisines(state_df):
  ...
  fig = px.pie(values=cuisines.values, names=cuisines.index, title='Most Popular Cuisines')
  return fig


def city_famous_cuisines(city_df):
    cuisines = city_df[0].value_counts().head(10)
    fig = px.pie(values=cuisines.values, names=cuisines.index, title='Most Popular Cuisines')
    return fig

def locality_famous_cuisines(locality_df):
    cuisines = locality_df[0].value_counts().head(10)
    fig = px.pie(values=cuisines.values, names=cuisines.index, title='Most Popular Cuisines')
    return fig

import plotly.express as px

# Define function to generate establishment type plot
def res_type(df, res_name):
    # Check if 'count' column exists in the DataFrame and DataFrame is not empty
    if 'count' in df.columns and not df.empty:
        # Extract top 10 establishment types based on count
        cuisines = df['count'].value_counts().head(10)
        # Generate pie chart
        fig = px.pie(values=cuisines, names=cuisines.index, hole=0.7)
        fig.update_layout(title=f'{res_name} Establishment Type')
        return fig
    else:
        # If 'count' column is missing or DataFrame is empty, return empty figure
        fig = px.pie(values=[], names=[], hole=0.7)
        fig.update_layout(title=f'{res_name} Establishment Type')
        return fig
...
def res_price_range(state_df, rest_name):
    if 'average_cost_for_two' in state_df.columns:
        price_range_counts = state_df['average_cost_for_two'].value_counts().head(10)
        fig = px.pie(values=price_range_counts, names=price_range_counts.index, hole=0.6)
        fig.update_layout(title=f'{rest_name} Price Range Distribution')
        return fig
    else:
        # If the 'average_cost_for_two' column is not found in the DataFrame
        # or if the DataFrame is empty, return an empty Figure
        fig = px.pie(values=[], names=[], hole=0.6)
        fig.update_layout(title=f'{rest_name} Price Range Distribution')
        return fig
